...
class SmartTv:
    ...
    def getCurrentChannel(self) -> int:
        self.ensureIsOn()
        return self.current # Return the current channel number
    
    def setChannel(self, channel:int) -> None:
        self.ensureIsOn()
        if channel < 1 or channel > self.channels: # Validate that the channel number is within the valid range
